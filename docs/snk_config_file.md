...
- `require_conda`: A boolean that controls whether the pipeline requires a conda environment. If set to `true`, the pipeline will fail if conda is not available.
