...
While `snk` is useful for managing workflows, using snk does add an extra step to typical install processes. User must first install snk before they can `snk install` your workflow. However, it is possible to build a workflow as a standalone package (relying on `pip` or `conda` to do the installation) and only using `snk` to dynamically generate the CLI. 
